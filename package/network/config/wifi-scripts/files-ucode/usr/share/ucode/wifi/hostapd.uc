...
export function setup(data) {
	...
	for (let k, interface in data.interfaces) {
		if (interface.config.mode == 'ap') {
			primary_ifname = interface.config.ifname;
			system(`iw dev ${primary_ifname} info >/dev/null 2>&1 || iw phy ${data.phy} interface add ${primary_ifname} type __ap`);
			if (interface.config.macaddr)
				system(`ip link set dev ${primary_ifname} address ${interface.config.macaddr}`);
			break;
		}
	}
	...
	let ret = global.ubus.call('hostapd', 'config_add', { iface: primary_ifname, config: file_name });
